    ...
    if start_date is not None and end_date is not None:
        ...
        if metrics is None:
            try:
                ...
                sys.exit(1)
        elif metrics is not None and type(metrics) is list:
            api_payload = {"metrics": metrics}
            try:
        ...
        data_metrics_split[["name", "type", "value"]] = pd.DataFrame(
        ...
        data_metrics_split = data_metrics_split[["unique_index", "name", "value","type"]]
...
def import_data_from_kinduct(
    ...
    api_import_endpoint = (
        f"https://{clientname}.{endpoint}/api/import/import_data"
    )
    authorization = f"{token_type} {access_token}"
    headers = {"Authorization": authorization, "Content-Type": "application/json"}
